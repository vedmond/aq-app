    ...
    <div>
      <div className={`${style.card} ${stateHelp ? style.active : ''}`}>
        <div className={style.card_popup}>
          ...
          <p className={style.text}>{`${dataQuiz[forStartId()].author}, ${
            dataQuiz[forStartId()].year
          }`}</p>
          <div className={style.btn_container}>
            <button onClick={callBack} className={`${style.btn_popup} ${style.btn1}`}>
              {finishedId === forStartId() ? 'Result' : 'Next'}
            </button>
          </div>
        </div>
      </div>
