...
export const Score = () => {
  ...
  for (let index = 1; index <= 12; index++) {
    ...
    artQuest =
      scoreObj[namePic].question + scoreObj[nameArt].question + artQuest;
  }
    ...
    <>
      <header className={style.header}>
        <div onClick={callBack} className={style.icon__arrow}>
          {" "}
          <div className={style.icon__arrow__block}></div> Score
        </div>
      ...
      <main className={style.main}>
        <div className={style.setting}>
          <h2 className={style.title}>
            Questions : <span className={style.item_an}>{artQuest}</span>
          </h2>
          <h2 className={style.title}>
            Correct : <span className={style.item_cor}>{artResult}</span>
          </h2>
          <h2 className={style.title}>
            Total -{" "}
            <span className={style.item_tot}>
              {total ? total : <MdSentimentVerySatisfied />}
            </span>{" "}
            %
          </h2>
        </div>
